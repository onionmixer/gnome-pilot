...
memoconduit_load_configuration (guint32 pilot_id)
{
	...
	if (c->registry) {
		c->source = e_pilot_get_sync_source (c->registry, E_SOURCE_EXTENSION_MEMO_LIST);
		if (!c->source) {
			GList *sources = e_source_registry_list_sources (c->registry, E_SOURCE_EXTENSION_MEMO_LIST);
			if (sources) {
...
memoconduit_save_configuration (EMemoConduitCfg *c)
{
	...
	e_pilot_set_sync_source (c->registry, E_SOURCE_EXTENSION_MEMO_LIST, c->source);
...
start_calendar_server (EMemoConduitContext *ctxt)
{
	...
	if (ctxt->cfg->source) {
		...
		if (!ctxt->client) {
			if (error) {
				WARN ("Failed to connect to memos: %s", error->message);
				g_error_free (error);
			}
			return -1;
		}
...
local_record_from_comp (EMemoLocalRecord *local, ECalComponent *comp, EMemoConduitContext *ctxt)
{
	...
	if (d_list) {
		...
		if (description && e_cal_component_text_get_value (description)) {
			local->memo->text = e_pilot_utf8_to_pchar (e_cal_component_text_get_value (description), ctxt->pilot_charset);
		}
		else{
			 ...
			 const gchar *pilot_charset)
{
	...
	if (!memo.text) {
		...
		txt3 = e_pilot_utf8_from_pchar(txt2, pilot_charset);
		sumText = e_cal_component_text_new (txt3, NULL);

		txt = e_pilot_utf8_from_pchar (memo.text, pilot_charset);
		...
		e_cal_component_text_free (sumText);
		free (txt);
		g_free(txt2);
		free(txt3);
	}
			...
			EMemoConduitContext *ctxt)
{
	...
	ctxt->ps = e_pilot_settings_new (ctxt->cfg->registry, E_SOURCE_EXTENSION_MEMO_LIST);
